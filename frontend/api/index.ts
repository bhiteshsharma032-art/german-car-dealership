...
const MOBILE_DE_CONFIG = {
  ...
  username: (process.env.MOBILEDE_API_USERNAME || 'dlr_dimitriosmikhovsky').trim(),
  password: (process.env.MOBILEDE_API_PASSWORD || 'kovoExT0mG3Y').trim(),
  customerId: (process.env.MOBILEDE_CUSTOMER_ID || '712285').trim(),
  timeout: parseInt((process.env.MOBILEDE_TIMEOUT || '15000').trim())
};
...
app.get('/api/inventory/test', async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      const errorMsg = typeof result.error === 'object' ? (result.error as any).message : (result.error || 'Connection failed');
      res.status((result.error as any)?.status || 500).json({ success: false, message: 'Mobile.de API connection failed', error: errorMsg });
    }
  } catch (error: any) {
...
app.get('/api/inventory/filters', async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      const errorMsg = typeof result.error === 'object' ? (result.error as any).message : (result.error || 'Failed to get filters');
      res.status((result.error as any)?.status || 500).json({ success: false, data: { brands: [], fuelTypes: [], bodyTypes: [], transmissions: [], conditions: [], years: [], priceRanges: [] }, error: errorMsg });
    }
  } catch (error: any) {
...
app.get('/api/inventory/search', async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      const errorMsg = typeof result.error === 'object' ? (result.error as any).message : (result.error || 'Search failed');
      res.status((result.error as any)?.status || 500).json({ success: false, data: [], total: 0, error: errorMsg, searchParams: req.query });
    }
  } catch (error: any) {
...
app.get('/api/inventory', async (req, res) => {
  try {
    ...
    const pageSize = 100; // mobile.de max per page
    ...
    totalAvailable = parseInt(firstSearchResult['search:total'] || '0');
    ...
    if (totalPages > 1) {
      const pagePromises = [];
      for (let p = 2; p <= Math.min(totalPages, 5); p++) { // max 5 pages = 500 vehicles
        pagePromises.push(
          makeMobileDeRequest('/search', {
            customerNumber: MOBILE_DE_CONFIG.customerId,
            'max-results': pageSize,
            'page.number': p,
            'page.size': pageSize
    ...
    console.log(`✅ Fetched ${allVehicles.length} of ${totalAvailable} total vehicles`);
...
app.get('/api/inventory/:id', async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      const errorMsg = typeof result.error === 'object' ? (result.error as any).message : (result.error || 'Failed to fetch vehicle');
      res.status((result.error as any)?.status || 500).json({ success: false, data: null, error: errorMsg, message: 'Failed to fetch vehicle from mobile.de' });
    }
  } catch (error: any) {
...
app.get('/api/admin/cars/:id', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      const errorMsg = typeof result.error === 'object' ? (result.error as any).message : (result.error || 'Failed to fetch car by ID');
      res.status((result.error as any)?.status || 500).json({ success: false, error: errorMsg });
    }
  } catch (error: any) {
...
app.get('/api/mobilede/test-connection', async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      const errorMsg = typeof result.error === 'object' ? (result.error as any).message : (result.error || 'Connection failed');
      res.status((result.error as any)?.status || 500).json({ success: false, message: 'Connection failed', error: errorMsg });
    }
  } catch (error: any) {
    res.status(500).json({ success: false, error: error.message });
  }
