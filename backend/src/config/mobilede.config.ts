...
export interface MobileDeConfig {
  baseUrl: string;
  username: string;
  password: string;
  customerId: string;
...
export function validateMobileDeConfig(): MobileDeConfig {
  const requiredVars = {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing required mobile.de environment variables: ${missing.join(', ')}\n` +
...
export const mobileDeConfig = validateMobileDeConfig();