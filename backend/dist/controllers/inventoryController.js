...
async function makeMobileDeRequest(endpoint, params = {}) {
    try {
        console.log('🔄 Mobile.de API Request:', { endpoint, params });
        const response = await axios_1.default.get(`${MOBILE_DE_CONFIG.baseURL}${endpoint}`, {
            headers: {
...
class InventoryController {
    ...
    async searchVehicles(req, res) {
        try {
            ...
            // Add search filters
            if (req.query.make)
                searchParams.make = req.query.make;
            if (req.query.model)
                searchParams.model = req.query.model;
            if (req.query.priceFrom)
                searchParams.priceFrom = req.query.priceFrom;
            if (req.query.priceTo)
                searchParams.priceTo = req.query.priceTo;
    ...
    async getFilterOptions(req, res) {
        try {
            ...
            if (result.success) {
                ...
                const years = [...new Set(vehicles.map(v => v.year).filter(y => y !== null))].sort((a, b) => b - a);
                res.json({
                    success: true,
                    data: {
                        brands,
